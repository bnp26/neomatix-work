...
//  videosink = gst_element_factory_make ("xvimagesink", "videosink");
    ...
    g_object_set(G_OBJECT(appsrc), "caps",
                 gst_caps_new_simple("video/x-raw",
                                     ...
                                     "height", G_TYPE_INT, 120, "framerate", GST_TYPE_FRACTION, 16, 1, NULL), NULL);
    g_object_set(G_OBJECT(appsrc), "stream-type", 0,    // GST_APP_STREAM_TYPE_STREAM
                 "format", GST_FORMAT_TIME, "is-live", TRUE, "max-bytes", 19200, NULL);
/*
	g_object_set (G_OBJECT (videosink), 
		"sync", FALSE, NULL);
*/
    g_object_set(G_OBJECT(videosink), "sync", FALSE, "async", TRUE, "host", address, "port", port, NULL);
